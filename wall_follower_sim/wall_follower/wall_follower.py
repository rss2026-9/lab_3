...
class WallFollower(Node):
    ...
    def __init__(self):
        ...
        self.error_pub = self.create_publisher(Float64, "/wall_error", 10)
        self.viz = None
        ...
        # ADDED: steering command low-pass to reduce oscillation (does NOT slow the car)
        self.STEER_LPF = 0.65          # ADDED: 0..1, higher = smoother (0.6-0.8 is typical)
        self.prev_steer_cmd = 0.0      # ADDED

        # ADDED: when wall is lost, steer gently toward the wall to reacquire (prevents drifting off track)
        self.RECOVERY_STEER = 0.10     # ADDED: radians-ish; small bias turn (0.06-0.14 works well)
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        # CHANGED: if wall not visible, bias steer toward the wall to reacquire instead of going straight
        if (not np.isfinite(a)) or (not np.isfinite(b)) or b >= 0.95 * msg.range_max:
            self._reset_state()
            rec_steer = float(self.SIDE) * float(self.RECOVERY_STEER)  # ADDED: steer toward wall side
            self._publish_drive(rec_steer)  # CHANGED
            return
        ...
        steer = clamp(steer, -self.MAX_STEER, self.MAX_STEER) #never command beyond the simulator’s max steering

        # ADDED: low-pass filter the final steering command for smoother behavior (reduces oscillation)
        steer = float(self.STEER_LPF * self.prev_steer_cmd + (1.0 - self.STEER_LPF) * steer)  # ADDED
        self.prev_steer_cmd = steer  # ADDED
    ...
    def _reset_state(self):
        ...
        self.prev_t = None
        self.d_filt = 0.0
        self.prev_steer_cmd = 0.0  # ADDED: reset steering LPF state on recovery
